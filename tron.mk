...
DOCKER_IMAGE = enixlan/tiler

VERSION  := $(shell git describe --tags --always)
GIT_HASH := $(shell git rev-parse HEAD 2> /dev/null)
BUILD_TS := $(shell date +%FT%T%z)

LDFLAGS:=-X 'github.com/loghole/tron/internal/app.ServiceName=tiler' \
		 -X 'github.com/loghole/tron/internal/app.AppName=github.com/enixlan/tiler' \
...
.PHONY: run-local
run-local:
	go run -ldflags "$(LDFLAGS)" cmd/tiler/main.go --local-config-enabled
